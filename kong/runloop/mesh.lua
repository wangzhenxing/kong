  ...
  for _, v in ipairs(protos) do
    ...
    elseif v == "http/1.1" or (v == "h2" and http2_enabled) then
      ...
      TODO: Figure out if current proxy listener directive has http2 allowed.
            ...
            This implementation enables all listeners to work with http2 when
            one of the listeners has it enabled. Currently we haven't figured
            out how to check this runtime. One possibility is to generate a
            separate server block for each listener, but that needs some
            discussion.
